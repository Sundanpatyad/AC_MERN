...
export default function VideoDetailsSidebar({ setReviewModal }) {
  ...
  return (
    <>
      <div className="flex h-[calc(100vh-3.5rem)] w-[320px] max-w-[350px] flex-col border-r-[1px] border-r-richblack-700 bg-surface">
        <div className="mx-5 flex flex-col items-start justify-between gap-2 gap-y-4 border-b border-line py-5 text-lg font-bold text-fg">
          <div className="flex w-full items-center justify-between ">
            ...
            <button
              ...
              className="flex h-[35px] w-[35px] items-center justify-center rounded-full bg-richblack-100 p-1 text-fg hover:scale-90"
              title="back"
            >
        ...
        <div className="h-[calc(100vh - 5rem)] overflow-y-auto">
          {courseSectionData.map((section, index) => (
            <div
              className="mt-2 cursor-pointer text-sm text-fg"
              onClick={() => setActiveStatus(section?._id)}
              key={index}
            >
              {/* Section */}
              <div className="flex justify-between bg-elevated px-5 py-4">
                  ...
                  {section.subSection.map((topic, i) => (
                    <div
                      className={`flex gap-3  px-5 py-2 ${videoBarActive === topic._id
                        ? "bg-white-200 font-semibold text-fg"
                        : "hover:bg-page"
                        } `}
                      key={i}
                      onClick={() => {
